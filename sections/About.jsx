...
const About = () => {
  ...
  return (
    <div className='px-16 bg-[rgb(15,20,24)] h-screen' id='about' >
      ...
      <div className='text-5xl w-[50%] mt-8 leading-tight ' ><h1 className='font-bold inline text-primary' >Building the future,</h1><span className='font-serif italic font-thin ' > one component at a time.</span></div>
      <span className=' grid w-[45%] grid-row-2 grid-cols-2 gap-y-8 absolute right-10' >
          {cards.map((item,idx) => (
            <AboutCard key={idx} title={CardData[idx].title} description={CardData[idx].description} value={item.component} />
          ))}
      </span>
      <div className='w-[42%] text-muted-foreground font-[500] mt-8 '>
